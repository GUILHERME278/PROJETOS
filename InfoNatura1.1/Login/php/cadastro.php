...
if (isset($_POST['enviar'])) { //Verifica se o botao Cadastrar foi clicado, para evitar que o usuario insira o endereco php direto na barra web
  ...
  $resultado = $connection->query($verificaemail);
...
{
  echo "<script>window.location.href = '../html/cadastro.html';</script>"; //Direciona para o formulario novamente 
}

if (mysqli_num_rows($resultado) > 0) {
  echo "<script>alert('Email ja Cadastrado!!!');</script>"; //Exibi um alerta  para que o usuario preencha os campos vazios
  echo "<script>window.location.href = 'cadastro.html';</script>"; //Atualiza a pagina ao apertar o ok no alert
  exit;
}
